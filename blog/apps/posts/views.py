...
class ListPostsView(StandardAPIView):
    def get(self, request, *args, **kwargs):
        ...
        Post.objects.filter(id__in=posts_shown).update(
...
class DetailPostView(StandardAPIView):
    def get(self, request, *args, **kwargs):
        ...
        return self.send_response(PostSerializer(Post.objects.get(id=id)).data)
    ...
    def post(self, request, format=None):
        ...
        if data.get('user').get('role') != 'editor':
            raise PermissionError(
                'You dont have the permissions to create a post.')
